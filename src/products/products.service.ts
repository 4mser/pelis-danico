...
export class ProductsService {
  ...
  async create(name: string, image?: string) {
    ...
    this.eventEmitter.emit('pet.interaction', { type: 'addProduct' as InteractionType });
    return product;
  }
  ...
  async update(
    ...
    if (!wasBought && updated.bought) {
      this.eventEmitter.emit('pet.interaction', { type: 'buyProduct' as InteractionType });
    }
    ...
    const likedOneNow =
      ...
      !updated.likeBoth;
    if (likedOneNow) {
      this.eventEmitter.emit('pet.interaction', { type: 'likeOne' as InteractionType });
    }

    const becameLikeBoth = !(wasLikeNico && wasLikeBarbara) && updated.likeBoth;
    if (becameLikeBoth) {
      this.eventEmitter.emit('pet.interaction', { type: 'likeBoth' as InteractionType });
    }
